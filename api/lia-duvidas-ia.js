  ...
  req.on('end', async () => {
    try {
      ...
      if (contemProibido(pergunta)) {
        const resposta = limparResposta('Na minha base, não encontrei esse procedimento como disponível na CR Laser®.);
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({ resposta }));
        return;
      }
      ...
      if (isPerguntaDiamond(pergunta)) {
        const resposta = limparResposta('O Bioestimulador Diamond é o bioestimulador usado na CR Laser®. Ele é indicado para estimular a produção de colágeno, melhorar a firmeza e a qualidade da pele, sempre conforme avaliação profissional.');
        res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
        res.end(JSON.stringify({ resposta }));
        return;
      }
      ...
      if (/\b(tem|faz|trabalha|oferece|possui|vende|realiza|fazem|oferecem|trabalham)\b/i.test(perguntaNorm)) {
        ...
        if (!achouPermitido) {
          const resposta = limparResposta('Na minha base, não encontrei uma confirmação segura sobre esse procedimento.);
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' });
          res.end(JSON.stringify({ resposta }));
          return;
        }
