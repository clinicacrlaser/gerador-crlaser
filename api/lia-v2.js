...
const RESPOSTA_URGENCIA_OFERTA_1 = 'Condição válida conforme campanha ativa e disponibilidade.';
...
function extrairFormaPagamentoContexto(contexto = {}) {
  return (
    contexto.formaPagamento ||
    contexto.pagamento ||
    contexto?.liaContext?.formaPagamento ||
    contexto?.liaContext?.pagamento ||
    null
  );
}
...
function sincronizarLiaContext(contextoEntrada = {}, texto = '') {
  ...
  const formaPagamento = formaMensagem || extrairFormaPagamentoContexto(contexto);
  ...
  const saida = {
    ...
    pagamento: formaPagamento || contexto.pagamento || null,
...
// ════ CÉREBRO CENTRAL V2 — Prioridade Única ════
// 1 HUMANO > 2 CONTATO > 3 POS_PAGAMENTO > 4 COMPRA > 5 PRECO
// 6 INFORMACAO_PROCEDIMENTO > 7 DUVIDA_TECNICA > 8 SAUDACAO > 9 FALLBACK
...
function classificarIntencaoCentral(texto = '', contexto = {}) {
  ...
  // 1. HUMANO — falar com pessoa/equipe
  if (detectarIntencaoHumano(texto)) return INTENCOES_CENTRAIS.HUMANO;

  // 2. CONTATO — onde fica, endereço, telefone, horário
  if (identificarIntencaoOperacional(texto) || detectarPerguntaOndeFicaDireta(texto)) {
    return INTENCOES_CENTRAIS.CONTATO;
  }

  // 3. POS_PAGAMENTO — apenas quando link já foi enviado
  if (
    contexto.linkEnviado &&
    (detectarPerguntaSuportePosLink(texto) || detectarPedidoContatoComprovante(texto) || detectarConfirmacaoPagamento(texto) || t.includes('comprovante') || t.includes('whatsapp'))
  ) {
    return INTENCOES_CENTRAIS.POS_PAGAMENTO;
  }

  // 4. COMPRA — ESTRITA: apenas quero comprar/fechar/pagar, vou querer
  if (detectarIntencaoCompraEstrita(texto)) return INTENCOES_CENTRAIS.COMPRA;

  // 4b. COMPRA via intenção de compra + procedimento detectado (lead quente)
  if (isPurchaseIntent(texto) && (detectarBaseProcedimentoAmbiguo(texto) || !!detectarProcedimento(texto))) {
    return INTENCOES_CENTRAIS.COMPRA;
  }

  // 5. PRECO — quanto custa, valor, oferta, promoção
  if (detectarPreco(texto)) return INTENCOES_CENTRAIS.PRECO;

  // 6. INFORMACAO_PROCEDIMENTO — perguntas informativas sobre tratamentos
  if (
    detectarConsultaInformativaBotox(texto) ||
    detectarPedidoMaisInformacaoTratamento(texto) ||
    detectarTemaBotoxFacial(texto) ||
    detectarPerguntaTemBotoxDireta(texto) ||
    detectarPerguntaAparelhoDireta(texto) ||
  ...
  // 7. DUVIDA_TECNICA — FAQ, correções, dúvidas específicas
  if (
    encontrarFaq(texto) ||
    encontrarSugestao(texto) ||
    encontrarCorrecao(texto) ||
    ...
    return INTENCOES_CENTRAIS.DUVIDA_TECNICA;
  }
...
function interpretarRespostaCurtaPorUltimaPergunta(texto = '', contexto = {}) {
  const t = normalizeText(texto);
  if (!['quero', 'sim', '1', '2', '3', '4', '5', 'ok'].includes(t)) return null;
...
function resolverFluxoCentral(pergunta = '', contexto = {}, extras = {}) {
  ...
  const cidadeDetectadaBase = extras.cidadeDetectada || contexto.liaContext?.cidade || contexto.cidadeAtual || contexto.cidadeCompra || contexto.cidade || null;
  const cidadeNumericaResolvida = !cidadeDetectadaBase && /^[1-5]$/.test(msg.trim()) &&
    ['fluxo_compra_aguardando_cidade_sistema', 'aguardando_cidade_para_confirmacao_oferta'].includes(contexto.intencao || '')
    ? ({ '1': 'brasilia', '2': 'campinas', '3': 'goiania', '4': 'palmas', '5': 'saopaulo' }[msg.trim()] || null) : null;
  const cidadeAtual = cidadeDetectadaBase || cidadeNumericaResolvida;
  ...
  if (decisaoCurta === 'PAGAMENTO_CARTAO') {
    if (!cidadeAtual) {
      const r = RESPOSTA_QUAL_UNIDADE_NUMERADA;
    ...
    const procResolvidoCartao = procEsp || resolverProcedimentoCompraPorContexto({ ...contexto, procedimentoBase: procBaseAtual || contexto.procedimentoBase });
...
(linkResp, cidadeAtual, procResolvidoCartao, 'cartao');
  ...
  if (decisaoCurta === 'ESCOLHA_1') {
    const procParaEscolha1 = (procFinalAtual && !['Botox', 'Ultraformer MPT', 'Lavieen'].includes(procFinalAtual))
      ? procFinalAtual
  ...
  if (etapaAtual === 'fluxo_compra_aguardando_pagamento' || etapaAtual === 'confirmacao_oferta_aguardando_pagamento') {
    ...
    if (formaPgto === 'cartao') {
      ...
      const procResolvidoAguardando = procEspCartao || resolverProcedimentoCompraPorContexto({ ...contexto, procedimentoBase: procBaseAtual || contexto.procedimentoBase });
  ...
  if (etapaAtual === 'fluxo_pagamento_aguardando_procedimento_cartao') {
    const cidadeCompra = contexto.cidadeCompra || cidadeAtual;
    // Aceita base names (ex: 'botox') e também nomes completos não-ambíguos
    let procResolvido = procFinalAtual && !['Ultraformer MPT', 'Lavieen'].includes(procFinalAtual) ? procFinalAtual : null;
    ...
    if (procResolvido && cidadeCompra) {
      let linkResp = gerarRespostaOfertaCampanha(procResolvido, cidadeCompra, 'texto');
      if (!linkResp) {
        const baseNm = normalizarProcedimentoBase(procResolvido);
        const canonical = baseNm ? (resolverProcedimentoPorBase(baseNm, procResolvido) || null) : null;
        if (canonical && canonical !== procResolvido) {
          linkResp = gerarRespostaOfertaCampanha(canonical, cidadeCompra, 'texto');
          if (linkResp) procResolvido = canonical;
        }
      }
      if (linkResp) {
  ...
  if (intencaoCentral === INTENCOES_CENTRAIS.COMPRA) {
    // Resolve procedimento final (inclusive Botox/Ultraformer/Lavieen genéricos)
    ...
    if (!cidadeAtual) {
      if (procBaseAtual === 'botox' || extras.procedimentoBaseMensagem === 'botox') {
        const r = `Perfeito 😊\n\nO Botox facial com direito a retorno no terço superior completo é uma das aplicações mais procuradas aqui na CR Laser®, buscando um resultado natural e seguro.\n\nPara eu te passar a condição correta, me diga qual unidade fica melhor pra você:\n\n1️⃣ Brasília\n2️⃣ Campinas\n3️⃣ Goiânia\n4️⃣ Palmas\n5️⃣ São Paulo`;
        return { resposta: r, contexto: ctxSaida({ intencao: 'fluxo_compra_aguardando_cidade_sistema', procedimentoBase: 'botox', status_compra: 'em andamento', ultimaPerguntaBot: r }) };
      }
      const r = RESPOSTA_QUAL_UNIDADE_NUMERADA;
      return { resposta: r, contexto: ctxSaida({ intencao: 'fluxo_compra_aguardando_cidade_sistema', status_compra: 'em andamento', ultimaPerguntaBot: r }) };
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (detectarIntencaoCompra(pergunta)) {
      ...
      const formaPagamentoCompra = extrairFormaPagamentoContexto(contexto);
      const procedimentoCompra = resolverProcedimentoCompraPorContexto({
        ...contexto,
        procedimento: procedimentoSelecionadoMensagem || contexto.procedimento,
        procedimento_selecionado: procedimentoSelecionadoMensagem || contexto.procedimento_selecionado
      });

      if (cidadeCompra && formaPagamentoCompra && procedimentoCompra) {
        ...
        if (respostaOferta) {
          return res.status(200).json({
            ...
            contexto: {
              ...
              pagamento: formaPagamentoCompra,
              procedimento: procedimentoCompra,
              procedimentoAtual: procedimentoCompra,
              procedimentoFinal: procedimentoCompra,
              procedimento_selecionado: procedimentoCompra,
              intencao: 'fluxo_pagamento_aguardando_confirmacao',
              aguardandoComprovante: true,
              aguardando_comprovante: true,
    ...
    if (contexto.intencao === 'fluxo_compra_aguardando_cidade_sistema') {
      const cidadeAtual = cidadeDetectada || contexto.cidadeCompra || contexto.cidadeAtual || contexto.cidade || contexto?.liaContext?.cidade || null;
      if (cidadeAtual) {
        const formaPagamentoSalva = extrairFormaPagamentoContexto(contexto);
        const procedimentoSalvo = resolverProcedimentoCompraPorContexto({
          ...contexto,
          procedimento: procedimentoSelecionadoMensagem || contexto.procedimento,
          procedimento_selecionado: procedimentoSelecionadoMensagem || contexto.procedimento_selecionado
        });

        if (formaPagamentoSalva === 'pix') {
          return res.status(200).json({
            ...
            contexto: {
              ...
              formaPagamento: 'pix',
              pagamento: 'pix',
              procedimento: procedimentoSalvo || contexto.procedimento,
              procedimentoAtual: procedimentoSalvo || contexto.procedimentoAtual,
              procedimentoFinal: procedimentoSalvo || contexto.procedimentoFinal,
              procedimento_selecionado: procedimentoSalvo || contexto.procedimento_selecionado,
              aguardandoComprovante: true,
              aguardando_comprovante: true,
              status_compra: 'em andamento'
            }
        ...
        if (formaPagamentoSalva === 'cartao' && procedimentoSalvo) {
          ...
          if (respostaOfertaSalva) {
            return res.status(200).json({
              ...
              contexto: {
                ...
                procedimentoAtual: procedimentoSalvo,
                procedimentoFinal: procedimentoSalvo,
                procedimento_selecionado: procedimentoSalvo,
                aguardandoComprovante: true,
                aguardando_comprovante: true,
                status_compra: 'em andamento'
              }
    ...
    if (contexto.intencao === 'fluxo_compra_aguardando_pagamento') {
      ...
      const formaPagamento = interpretarFormaPagamentoPorRespostaCurta(pergunta) || detectarFormaPagamento(pergunta) || extrairFormaPagamentoContexto(contexto);
      ...
      if (formaPagamento === 'cartao') {
        const procedimentoDoContextoDireto = resolverProcedimentoCompraPorContexto(contexto);
        if (procedimentoDoContextoDireto) {
          ...
          if (respostaOfertaDireta) {
            return res.status(200).json({
              ...
              contexto: {
                ...
                cidadeAtual: cidadeCompra,
                cidadeCompra,
                procedimento: procedimentoDoContextoDireto,
                procedimentoAtual: procedimentoDoContextoDireto,
                procedimentoFinal: procedimentoDoContextoDireto,
                procedimento_selecionado: procedimentoDoContextoDireto,
                aguardandoComprovante: true,
                aguardando_comprovante: true,
    ...
    if (detectarPedidoMaisInformacaoTratamento(pergunta)) {
      ...
      return res.status(200).json({
        resposta: `Perfeito 😊\n\nEscolha o procedimento no sistema e finalize sua compra diretamente.\n\n${RESPOSTA_ESCOLHA_DIRETA_PADRAO}`,
        contexto: {
          ...contexto,
          intencao: 'aguardando_interesse'
        }
...
// ════ COMPRA ESTRITA (Regra 6) ════
// Só as frases explícitas de intenção de compra — sem "quero aproveitar", "quero essa oferta"
function detectarIntencaoCompraEstrita(texto = '') {
  const t = normalizeText(texto);
  return (
    t.includes('quero comprar') ||
    t.includes('quero fechar') ||
...
function isPurchaseIntent(texto = '') {
  const t = normalizeText(texto);
  return (
    t.includes('comprar') ||
    ...
    t.includes('quero pagar') ||
    t.includes('ver oferta') ||
    t.includes('quero a oferta') ||
    t.includes('me manda a oferta') ||
    t.includes('oferta') ||
    t.includes('finalizar') ||
    t.includes('pode mandar') ||
